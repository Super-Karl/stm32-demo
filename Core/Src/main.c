...
int main(void) {
    ...
    while (1) {
        /* USER CODE END WHILE */
        GPIO_PinState key_state = HAL_GPIO_ReadPin(K1_GPIO_Port, K1_Pin);
        if (key_state == GPIO_PIN_RESET) {
            HAL_Delay(50);
            key_state = HAL_GPIO_ReadPin(K1_GPIO_Port, K1_Pin);
            // 当满足上一个状态为未按下，当前状为已按下，那么表示按键的下降沿，则触发状态转换
            if (last_state == GPIO_PIN_SET && key_state == GPIO_PIN_RESET) {
                // 设置上一个状态为已按下
